...
def findCloseColor(color):
    ...
    for key in pygame.color.THECOLORS:
        score = 0
        for i in range(3):
            score += abs(pygame.color.THECOLORS[key][i] - color[i])
        if score < bestScore:
            bestScore = score
            bestName = key
...
rSlider = colorSlider(100,400,600,30,"R")
gSlider = colorSlider(100,450,600,30,"G")
bSlider = colorSlider(100,500,600,30,"B")
...
while True:
    ...
    closeColor = findCloseColor(color)
    pygame.draw.rect(DISPLAYSURF,(0,0,0),(100,100,600,200),5)

    pygame.draw.rect(DISPLAYSURF,color,(100,100,300,200))
    pygame.draw.rect(DISPLAYSURF, pygame.Color(closeColor), (400, 100, 300, 200))

    colorName = labelFont.render("Closest Color: "+closeColor,True,(0,0,0))
    colorNameRect = colorName.get_rect()
    colorNameRect.center = (400,350)
    DISPLAYSURF.blit(colorName,colorNameRect)
    pygame.display.flip()